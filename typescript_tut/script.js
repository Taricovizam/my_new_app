...
const mergeArr = (a1, a2) =>
    a1.map(itm => ({
        ...a2.find((item) => (item.name === itm.name)),
        ...itm
    }));
// const mergeArr = (a1, a2) =>
//     a1.map(itm => ({
...
//         ...itm
//     }));
const newArr = mergeArr(countries, allTimeZone)
let countryTimeZone = newArr.map(v => ({ [v.id]: v.Timezone }))
let countryUTC = newArr.map(v => ({ [v.id]: v.UTC }))

// let countryTimeZone = countries.map(v1 => {
//     let currCountry = v1.name
//     return (...allTimeZone.find(v2=> currCountry === v2  ))
// })
...
// let fullCountryNames = countries.map(v => ({ [v.id]: { countryName: v.name } }))
let countryNameFull = countries.map(v => ({ [v.id]: v.name }))
let countryNameAlpha2 = countries.map(v => ({ [v.id]: v.code }))
let countryNameAlpha3 = countries.map(v => ({ [v.id]: v.code3 }))
let countryCapital = countries.map(v => ({ [v.id]: v.capital }))
let countryCurrencyNumberCode = countries.map(v => ({ [v.id]: v.number.length === 1 ? "00" + v.number ? v.number === 2 : "0" + v.number : v.number }))
let countryPhoneCode = countries.map(v => ({ [v.id]: "+" + v.phone }))
